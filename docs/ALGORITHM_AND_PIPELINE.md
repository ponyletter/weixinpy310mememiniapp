...
> ⚠️ **合规勘误（2026-09-22）**：文中任何“审核后由后端放开未审核功能”的旧建议均已废止。功能变更必须随新版本重新提交审核。
...
### 5. 等待期互动彩蛋 (Snake Game 趣味互动)
- 生图通常需要 25~35 秒，在出图等待期嵌入原生 Canvas 贪吃蛇互动，彻底消除用户等待焦虑，显著提升留存率。

---

## 八、ChatGPT Images 2.5 特色草图功能 (Online Sketch-to-Image / SketchUP)

### 1. 功能背景与业务价值
ChatGPT Images 2.5 (gpt-image-2) 拥有行业顶尖的**草图理解与线条细化能力（Sketch-to-Image / SketchUP）**。用户仅需随手勾勒几笔火柴人姿态、动物轮廓或表情神态，AI 即可深度解析线条构图，自动补全服饰、上色与材质，并生成动作完全符合草图意图的 16 帧连贯表情包。

### 2. 纯原生 Canvas 在线涂鸦板实现
前端集成无第三方依赖的纯原生 Canvas 手绘涂鸦板（支持 PC 鼠标与手机触控屏）：
- **工具箱**：
  - 铅笔（Pencil）与橡皮擦（Eraser）平滑切换；
  - 笔触粗细无级调节（2px ~ 30px）；
  - 经典手绘 6 色调色盘（碳黑、正红、宝蓝、翠绿、明黄、亮粉）；
  - 单步撤销（Undo）与一键清空（Clear）；
- **像素级触控适配**：
  - 监听 `pointerdown / pointermove / pointerup` 与 `touchstart / touchmove / touchend`；
  - 自动绑定 `getBoundingClientRect()` 精准补偿滚动与 CSS 缩放偏移；
  - 设置 `touch-action: none` 防止手机端下拉刷新冲突。
- **一键应用与人设打通**：
  - 点击“确认使用此草图”，前端将 Canvas 导出为透明 PNG Blob，无缝转换为 `selectedRefImage`，并在缩略图上打上 `🎨 手绘草图` 专属标签；
  - 携带 `is_sketch = true` 参数提交给后端。

### 3. 草图专用提示词调优管线
在 `backend/app/core/prompt_templates.py` 中，当检测到 `is_sketch = True` 时，动态切换为 Images 2.5 草图强化提示词：
```text
【手绘草图精细化与角色赋予规范】：
1. 用户上传了一张手绘草图/构图轮廓（Sketch）。请将其作为最高优先级的姿态、形体构图与分镜参考；
2. 严格依据草图中的线条勾勒与动作体态，将其升华细化为精致、生动、色彩饱满且线条干净的完整二次元/Q版卡通表情包角色；
3. 保留草图中的神态特征与骨骼动势，在此基础上拓展完整的 4×4 连贯律动动画；
4. 画面背景保持纯白（RGB 255, 255, 255），无杂色，无网格分割线。
```

---

## 九、微信小程序个人主体合规方案与折叠彩蛋设计

### 1. 微信小程序个人主体审查红线
微信官方对 **“个人主体”** 小程序有着极度严苛的类目限制：
- **核心禁区**：**个人主体绝对禁止经营或包含任何形式的“游戏（Game）”**；
- **审查机制**：微信提审时，机器静态扫描与人工审核员会检索页面文本（关键词如“游戏”、“贪吃蛇”、“Game”、“得分”等）及默认可见的 Canvas 游戏画面。一经发现，立即以“服务类目与实际运营内容不符（涉嫌个人制作小游戏）”驳回；
- **空间冲突**：在移动端竖屏有限的空间内，若等待区默认摆放大型游戏界面，会严重遮挡关键的生图进度与操作按钮。

### 2. 折叠彩蛋交互架构 (Hidden Easter Egg)
针对上述约束，我们将趣味互动重构为 **“极简进度优先 + 棋盘格隐藏彩蛋”** 模式：

```mermaid
flowchart TD
    Start["发起 AI 生图任务"] --> Waiting["进入等待状态 (默认紧凑卡片)"]
    
    subgraph 默认展示区 ["默认合规紧凑视图 (节省 60% 屏幕空间)"]
        ProgressBar["tqdm 动态进度条 & 真实耗时/倒计时"]
        StepText["当前执行物理阶段 (生成/切片/去底/封装)"]
        Checkerboard["等待棋盘格动效预览框<br>(轻量文字: 点击棋盘格探索趣味动效 ✨)"]
    end
    ...
    Checkerboard -- "用户主动点击棋盘格" --> Unfold["展开原生 Canvas 贪吃蛇彩蛋"]
    Unfold -- "再次点击收起" --> Checkerboard
    
    subgraph 微信小程序发布策略 ["微信发布双保险"]
        AuditMode["审核期 (Audit Mode):<br>配置 is_audit_mode=true<br>彻底移除点击展开事件，仅展示纯净进度条"]
        ReleaseMode["运营期 (Release Mode):<br>云端动态下发配置开启彩蛋<br>用户主动探索触发，合规且有趣"]
    end
```

### 3. 实现细节与微信小程序端移植策略
1. **界面与文本彻底脱敏**：
   - 默认视图不出现“贪吃蛇”、“小游戏”等敏感词汇；
   - 占位框仅提示 `✨ 点击棋盘格探索趣味动效` 或 `🎨 等待时点击这里打发时间`；
2. **按需渲染与性能节约**：
   - 游戏容器 `#snakeGameBox` 默认 `display: none`；
   - 只有在用户主动点击棋盘格展开后，才初始化游戏引擎循环（`requestAnimationFrame`），避免后台消耗 CPU 与手机电量；
   - 随时点击“收起动效”即可收回折叠状态，恢复纯粹的进度条展示；
3. **微信小程序端（uni-app / 微信原生）无缝对齐**：
   - 在小程序端，`<canvas type="2d">` 实现相同的渲染逻辑；
   - 在 `App.vue` 或全局状态中设置 `showWaitingEgg: false`，提审时走微信接口或由后端云配置控制；提审通过后，服务端一键放开，免再次提交审核更新！
